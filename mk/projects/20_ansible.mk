...
$(ANSIBLE_HOSTS_PLAYBOOK_DIR):
	...
	@mkdir -p $@
	@cp $(FILES_DIR)/ansible/examples/host-playbook.yml $@/example-host.yml

$(ANSIBLE_GROUPS_PLAYBOOK_DIR):
	@echo "Initializing Ansible groups playbook directory ($@)."
	@mkdir -p $@
	@cp $(FILES_DIR)/ansible/examples/group-playbook.yml $@/example_group.yml
...
$(ANSIBLE_HOST_VARS_DIR):
	...
	@mkdir -p $@
	@cp $(FILES_DIR)/ansible/examples/host-vars.yml $@/example-host.yml

$(ANSIBLE_GROUP_VARS_DIR):
	@echo "Initializing Ansible group variables directory ($@)."
	@mkdir -p $@
	@cp $(FILES_DIR)/ansible/examples/group-vars.yml $@/example_group.yml
